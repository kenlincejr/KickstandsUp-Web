  ...
  return (
    <section className="tool-page route-detail-page">
      ...
      <header className="tool-header">
        ...
        {!route.grantId ? <button className="secondary-button" onClick={() => void toggleFavorite()} type="button">{route.isFavorite ? '★ Favorited' : '☆ Add favorite'}</button> : null}
      </header>
      ...
      <div className="route-detail-grid">
        ...
        <aside className="route-action-card"><span>RIDE IT</span><h2>Send this route to Google Maps.</h2><p>This handoff does not create another paid KSU route calculation. Longer routes are split into portable legs without dropping a stop.</p>{handoffs.length ? <div className="handoff-list">{handoffs.map((handoff) => <a className="primary-button" href={handoff.url} key={handoff.segmentNumber} rel="noreferrer" target="_blank">{handoff.segmentCount > 1 ? `Open leg ${handoff.segmentNumber} of ${handoff.segmentCount}` : 'Open in Google Maps'}</a>)}</div> : <p className="notice">This route needs at least a start and finish before Maps handoff is available.</p>}{route.provenance ? <p className="route-provenance">{route.provenance}</p> : null}</aside>
      </div>
    </section>
  );
}
